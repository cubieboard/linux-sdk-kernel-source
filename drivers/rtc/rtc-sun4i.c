...
#define SUNXI_ALARM_CONFIG_REG				(0x170)
...
static void sunxi_rtc_setaie(int to)
{
	...
	alarm_irq_val = readl(sunxi_rtc_base + SUNXI_ALARM_EN_REG);//0x114
	switch(to){
		case 1:
		writel(0x00000001, sunxi_rtc_base + SUNXI_ALARM_CONFIG_REG);//170
		alarm_irq_val |= RTC_ALARM_COUNT_INT_EN;		//0x00000100
	    writel(alarm_irq_val, sunxi_rtc_base + SUNXI_ALARM_EN_REG);//0x114

		alarm_irq_reg = readl(sunxi_rtc_base + SUNXI_ALARM_INT_CTRL_REG);//0x118
		alarm_irq_reg |= RTC_ENABLE_CNT_IRQ;
		writel(alarm_irq_reg, sunxi_rtc_base + SUNXI_ALARM_INT_CTRL_REG);//0x118
		break;
		case 0:
		default:
		alarm_irq_val = 0x00000000;
	    writel(alarm_irq_val, sunxi_rtc_base + SUNXI_ALARM_EN_REG);//0x114
	    writel(alarm_irq_val, sunxi_rtc_base + SUNXI_ALARM_INT_CTRL_REG);//0x118
	    writel(3, sunxi_rtc_base + SUNXI_ALARM_INT_STATUS_REG);//0x11c
		break;
	}
}
...
#define _ALARM_CNT_DAY(x)       (((x)>>24) & 0xff)
#define _ALARM_CNT_HOUR(x)      (((x)>>16) & 0x1f)
#define _ALARM_CNT_MIN(x)       (((x)>>8)  & 0x3f)
#define _ALARM_CNT_SEC(x)       (((x)>>0)  & 0x3f)
static void sunxi_rtc_shutdown(struct platform_device *dev)
{
    int alarm_time;
	void __iomem *base = sunxi_rtc_base;
    alarm_time = readl(base + SUNXI_RTC_ALARM_DD_HH_MM_SS_REG);

    if(alarm_time > 10){     /* need to fir after shutdown */
        pr_info( "%s: fir alarm after: %03d[day] %02d[hour] %02d[minute] %02d[second]\n",
            __func__,
           _ALARM_CNT_DAY(alarm_time), _ALARM_CNT_HOUR(alarm_time),
            _ALARM_CNT_MIN(alarm_time), _ALARM_CNT_SEC(alarm_time) );
        sunxi_rtc_setaie(1);
    }else{
        pr_info("%s: disable wakeup output.", __func__);
       sunxi_rtc_setaie(0);
    }
}
