...
#define ctp_wakeup_no  4
#define ctp_int_port 3
...
*/
        u32 gpio_status;
	gpio_direction_output(ctp_wakeup_no,1);

        if(status == 0){               

                if(ms == 0) { 
                	gpio_set_value(ctp_wakeup_no, 0);
                }else {
                        gpio_set_value(ctp_wakeup_no, 0);
                        msleep(ms);
                        gpio_set_value(ctp_wakeup_no, 1);
                }
        }
        if(status == 1){
                if(ms == 0) {
...
static void gsl_load_fwx680(struct i2c_client *client)
{
	...
	u8 *cur = buf + 1;
	u32 source_line = 0;
	u32 source_len = gslx680_fw_grp[0].size;
	const struct fw_data *fw =  gslx680_fw_grp[0].fw;
...
{
	printk("source data :id=%d,x=%d,y=%d\n",id,x,y);
	...
	printk("report data :id=%d,x=%d,y=%d\n",id,x,y);

	if(x>=SCREEN_MAX_X||y>=SCREEN_MAX_Y)
	{
	#ifdef HAVE_TOUCH_KEY
	printk("this is report_key......********************\n");
		report_key(ts,x,y);
	#endif
		return;
	}
...
//	printk("[tp-gsl] tmp1=%x\n",tmp1);
	...
	if(0 == touches){
		...
		input_mt_sync(ts->input);
	#ifdef HAVE_TOUCH_KEY
		printk("this is report android 4.0 and touch key**************8\n");
		if(key_state_flag){
...
static irqreturn_t gsl_ts_irq(int irq, void *data)
{
	struct gsl_ts *ts = data;
//	printk("==========GSLX680 Interrupt============\n");
//        printk("Enter work\n");
//        printk(KERN_DEBUG"this is debug: function=%s, line=%d\n",__FUNCTION__,__LINE__);
//	printk(KERN_DEBUG"ts=%p******************\n",ts);
...
static int gsl_ts_resume(struct i2c_client *client)
{
	struct gsl_ts *ts = i2c_get_clientdata(client);
  	...
  	printk("CONFIG_HAS_EARLYSUSPEND:%s,start\n",__func__);
...
static void gsl_ts_early_suspend(struct early_suspend *h)
{
	...
	printk("CONFIG_HAS_EARLYSUSPEND:Enter %s\n", __func__);
...
{
        printk(KERN_DEBUG"this is debug: function=%s, line=%d\n",__FUNCTION__,__LINE__);
	printk(KERN_DEBUG"ts_init=%p,ts_int->work=%p,ts_init->wq=%p\n",ts_init,ts_init->work,ts_init->wq);
	gslX680_chip_init();
...
*/
	if(gpio_request(ctp_int_port,"ctp_int_port")){
		printk(KERN_DEBUG ": %s:%i\n", __FUNCTION__, __LINE__);
	}
	IRQ_NUM  = gpio_to_irq(ctp_int_port);
        printk(KERN_DEBUG"this is debug: function=%s, line=%d\n",__FUNCTION__,__LINE__);
	printk(KERN_DEBUG"ts_init=%p,ts_int->work=%p,ts_init->wq=%p\n",ts_init,ts_init->work,ts_init->wq);

	int ret_irq = request_irq(IRQ_NUM, gsl_ts_irq, IRQF_TRIGGER_FALLING | IRQF_SHARED | IRQF_DISABLED, "ctp_int_port",ts_init);
	if(ret_irq){
...
//	sw_gpio_irq_free(int_handle);
 	free_irq(IRQ_NUM,ts_init);
	gpio_free(ctp_int_port);
#else
	del_timer(&ts->gsl_timer);
#endif
	destroy_workqueue(ts->wq);
...
/*
	...
	if(!ctp_get_system_config()){
                nit_platform_resource,printk("%s:read config fail!\n",__func__);
                return ret;
        }
*/
	...
	if(gpio_request(ctp_wakeup_no,"ctp_wakeup_no")){
		printk("gpio wakeup error,*** %s\n",__func__);
	}
	ctp_wakeup(ctp_wakeup_no, 0, 10);
